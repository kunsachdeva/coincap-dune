...
export default function Table({top20CryptoCurrencies}: Props) {
  ...
  const displayCoins = sort === "asc"? top20CryptoCurrencies.sort((coinA, coinB) => coinA.priceUsd - coinB.priceUsd) : sort === "dec" ? top20CryptoCurrencies.sort((coinA, coinB) => coinB.priceUsd - coinA.priceUsd) : top20CryptoCurrencies
   ...
   return <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                {displayCoins.map((cryptoCurrency: CryptoCurrency) => <TableRow key={cryptoCurrency.id} cryptoCurrency={cryptoCurrency} />)}
                </tbody>
            </table>
