...
<div className="max-h-420 w-full bg-white rounded-lg shadow-sm dark:bg-gray-800 p-4 md:p-6 mb-6">
 {firstPoint && <div className="flex justify-between">
    <div>
      <h5 className="leading-none text-3xl font-bold text-gray-900 dark:text-white pb-2">${(Number(lastPoint)).toFixed(2)}</h5>
      <p className="text-base font-normal text-gray-500 dark:text-gray-400">Historical price chart</p>
    </div>
    <div
      className="flex items-center px-2.5 py-0.5 text-base font-semibold text-gray-500 dark:text-gray-500 text-center">
      {(100*(Number(lastPoint)-Number(firstPoint))/Number(firstPoint)).toFixed(2)}%      
    </div>
  </div>} 
